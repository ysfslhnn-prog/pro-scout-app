...
# --- 1. PREMIUM UI CONFIG ---
st.set_page_config(page_title="Pro-Scout v34.0", layout="wide")

# Takım Verileri (Renkler ve Logolar)
TEAM_DATA = {
    ...
    'Real Madrid': {'colors': ('#ffffff', '#0047a0'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/5/56/Real_Madrid_CF_logo.svg'},
    ...
    'Liverpool': {'colors': ('#c8102e', '#f6eb61'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/0/0c/Liverpool_FC.svg'},
...
def engine_v34(ev_b, dep_b, h_ev_b, h_dep_b, ev_f, dep_f):
...
# --- 3. INFOGRAPHIC ---
def draw_v34_card(ev, dep, ms, iy, htft, kg, u25, p1, p2):
    fig, ax = plt.subplots(figsize=(6, 9.5), facecolor=p1)
    ax.axis('off')
    plt.text(0.5, 0.94, "PRO-SCOUTMASTER v34.0", color='white' if p1 != '#ffffff' else 'black', fontsize=22, ha='center', weight='bold')
...
# --- 4. MASTER UI ---
st.markdown("<h1 style='text-align:center; color:#1e293b;'>🏆 PRO-SCOUT MASTER v34.0</h1>", unsafe_allow_html=True)

lig_sel = st.selectbox("🌍 LİG SEÇİN", list(LIGLER.keys()))
data_v34 = load_v34(LIGLER[lig_sel])

if data_v34:
    mega, m_ev, m_dep, m_ht_ev, m_ht_dep, le, active_teams = data_v34
    ...
    if st.button("📊 DERİN ANALİZİ BAŞLAT", use_container_width=True):
        # NameError Çözümü: TEAM_DATA kullanıldı
        ...
        evw, ber, depw, u15, u25, kg, htft, top8, surp = engine_v34(fe, fd, he, hd, ev_f, dep_f)

        # RENKLİ GRADIENT BAŞLIK VE LOGOLAR
        st.markdown(f"""<div style="background:linear-gradient(90deg, {p1} 0%, {p2} 100%); color:white; padding:25px; border-radius:15px; text-align:center; box-shadow: 0 4px 10px rgba(0,0,0,0.3); margin-bottom:20px;">
        <div style="display:flex; justify-content:space-evenly; align-items:center;">
        <img src="{ev_logo}" width="70" style="background:white; border-radius:50%; padding:5px;"/>
        <h2 style="margin:0; text-transform:uppercase; letter-spacing:2px;">{ev_t} - {dep_t}</h2>
        <img src="{dep_logo}" width="70" style="background:white; border-radius:50%; padding:5px;"/>
        </div>
        <div style="margin-top:10px; font-weight:700;">FORM: %{ev_f*100:.0f} vs %{dep_f*100:.0f}</div></div>""", unsafe_allow_html=True)
        ...
        with c_res1:
            ...
            <div style="font-size:60px; font-weight:900; color:#1e293b; margin:10px 0;">{int(np.round(fe))}-{int(np.round(fd))}</div>
        ...
        report = draw_v34_card(ev_t, dep_t, f"{int(np.round(fe))}-{int(np.round(fd))}", f"{int(np.round(he))}-{int(np.round(hd))}", htft[0][0], kg, u25, p1, p2)
