...
# --- 1. PREMIUM UI & LOGO DATA ---
st.set_page_config(page_title="Pro-Scout v35.0", layout="wide")

# Daha güvenilir logo linkleri (PNG/JPG tercih edildi)
TEAM_DATA = {
    'Galatasaray': {'colors': ('#890e10', '#fdb912'), 'logo': 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Galatasaray_Sports_Club_Logo.svg/200px-Galatasaray_Sports_Club_Logo.svg.png'},
    ...
    'Real Madrid': {'colors': ('#0047a0', '#ffffff'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/thumb/5/56/Real_Madrid_CF_logo.svg/200px-Real_Madrid_CF_logo.svg.png'},
    'Barcelona': {'colors': ('#a50044', '#004d98'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/thumb/4/47/FC_Barcelona_%28crest%29.svg/200px-FC_Barcelona_%28crest%29.svg.png'},
    'Man City': {'colors': ('#6caddf', '#1c2c5b'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/thumb/e/eb/Manchester_City_FC_badge.svg/200px-Manchester_City_FC_badge.svg.png'},
    'Arsenal': {'colors': ('#ef0107', '#061922'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/thumb/5/53/Arsenal_FC.svg/200px-Arsenal_FC.svg.png'},
    'Liverpool': {'colors': ('#c8102e', '#f6eb61'), 'logo': 'https://upload.wikimedia.org/wikipedia/en/thumb/0/0c/Liverpool_FC.svg/200px-Liverpool_FC.svg.png'},
    'Bayern Munich': {'colors': ('#dc052d', '#0066b2'), 'logo': 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/1b/FC_Bayern_M%C3%BCnchen_logo_%282017%29.svg/200px-FC_Bayern_M%C3%BCnchen_logo_%282017%29.svg.png'}
}
...
# --- 2. 3 YILLIK ANALİZ VE EĞİTİM MOTORU ---
@st.cache_data
def master_load_v35(lig_kodu):
    mega = pd.DataFrame()
    current_teams = []
    # 3 Yıllık Genel Analiz Döngüsü
    for s in ["2324", "2425", "2526"]:
        try:
            ...
            # Sadece en güncel sezonun takımlarını filtrelemek için
...
def engine_v35(ev_b, dep_b, h_ev_b, h_dep_b, ev_f, dep_f):
...
# --- 3. INFOGRAPHIC ---
def draw_v35_card(ev, dep, ms, iy, htft, kg, u25, p1, p2):
    fig, ax = plt.subplots(figsize=(6, 9), facecolor=p1)
    ax.axis('off')
    plt.text(0.5, 0.94, "PRO-SCOUT MASTER v35.0", color='white' if p1 != '#ffffff' else 'black', fontsize=22, ha='center', weight='bold')
...
# --- 4. MASTER UI ---
st.markdown("<h2 style='text-align:center;'>🏆 PRO-SCOUT MASTER v35.0</h2>", unsafe_allow_html=True)

lig_sel = st.selectbox("🌍 LİG SEÇİN", list(LIGLER.keys()))
data_v35 = master_load_v35(LIGLER[lig_sel])

if data_v35:
    mega, m_ev, m_dep, m_ht_ev, m_ht_dep, le, active_teams = data_v35
    col_sel1, col_sel2 = st.columns(2)
    ev_t = col_sel1.selectbox("🏠 EV SAHİBİ", active_teams)
    dep_t = col_sel2.selectbox("🚀 DEPLASMAN", active_teams)
    ...
    if st.button("📊 DERİN ANALİZİ BAŞLAT", use_container_width=True):
        ...
        evw, ber, depw, u15, u25, kg, htft, top8, surp = engine_v35(fe, fd, he, hd, ev_f, dep_f)

        # LOGO VE BAŞLIK ALANI (Boyutlandırma düzeltildi)
        st.markdown(f"<div style='background:linear-gradient(90deg, {p1} 0%, {p2} 100%); color:white; padding:20px; border-radius:15px; text-align:center; margin-bottom:20px;'>", unsafe_allow_html=True)
        h_col1, h_col2, h_col3 = st.columns([1, 2, 1])
        with h_col1: st.image(ev_logo, width=80)
        with h_col2: st.markdown(f"### {ev_t.upper()} vs {dep_t.upper()} <br><small>FORM: %{ev_f*100:.0f} vs %{dep_f*100:.0f}</small>", unsafe_allow_html=True)
        with h_col3: st.image(dep_logo, width=80)
        ...
        with c_res1:
            ...
            st.markdown("<h4 style='margin-top:20px;'>📊 EN OLASI 8 SKOR</h4>", unsafe_allow_html=True)
        ...
        report = draw_v35_card(ev_t, dep_t, f"{int(np.round(fe))}-{int(np.round(fd))}", f"{int(np.round(he))}-{int(np.round(hd))}", htft[0][0], kg, u25, p1, p2)
