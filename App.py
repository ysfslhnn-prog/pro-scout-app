# PRO-SCOUT v57 GOD MODE - HATASIZ, PREMIUM UI, RADAR + BANKO AI
...
def load_data(code):
    ...
    for season in ["2324","2425","2526"]:
        try:
            ...
            if all(col in tmp.columns for col in required_cols):
                df = pd.concat([df, tmp[required_cols]], ignore_index=True)
        except:
            continue
    if df.empty:
        st.warning("Seçilen lig için veri bulunamadı!")
        return None

    # Form ve savunma hesapları
